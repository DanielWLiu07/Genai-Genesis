const LOCAL_AUTH_EMAIL_KEY = 'lotus_local_auth_email';
...
export function setLocalAuth(email: string) {
  ...
  document.cookie = 'lotus_authed=1; path=/; max-age=604800; SameSite=Lax';
}

export function clearLocalAuth() {
  if (typeof window === 'undefined') return;
  localStorage.removeItem(LOCAL_AUTH_EMAIL_KEY);
  document.cookie = 'lotus_authed=; path=/; max-age=0';
}
